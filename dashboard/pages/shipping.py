...
현지배송이 시작된 주문을 모아 배송대행지(에코트랜스) 신청서 xlsx를
생성하고, 국제배송 송장이 나오면 등록한 뒤, 마지막으로 네이버 스마트스토어에
발송 처리를 한다 (TBD Pipeline.md의 배송 단계 규칙 그대로).

"스마트스토어 등록"만 실제 라이브 API 쓰기(되돌리기 어려움)라 live_write_button +
confirm_dialog + 미리보기 토글을 쓴다. xlsx 생성/국제송장 등록은 NocoDB에만
...
def shipping_page() -> None:
  with layout.frame(active_path="/shipping"):
    ...
    def _refresh():
      ...
      rows = of.get_all()
      awaiting_application = [r for r in rows if r["fields"].get("local_tracking_number") and not r["fields"].get("iecot_applied_at")]
    ...
    def _render_application_section(rows: list[dict]):
      ...
      for r in rows:
        ...
        with ui.row().classes("w-full gap-3 items-center tbd-card").style("padding: 12px 16px;"):
          ...
          ui.label(f"{f.get('naver_product_name', '')[:40]} · 현지송장 {f.get('local_tracking_number', '')}").classes(
              "text-sm flex-1"
          )
      ...
      for r in rows:
        ...
        with ui.row().classes("w-full gap-3 items-end tbd-card").style("padding: 16px;"):
          with ui.column().classes("gap-0 flex-1"):
            ...
            ui.label(f"현지송장 {f.get('local_tracking_number', '')}").classes("text-xs text-tbd-text-secondary")
