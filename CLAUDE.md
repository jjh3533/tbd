...
| `common_settings.py`/`common_settings.json` | 브랜드 로고/문구 + 구매·배송 안내 문구(초기불량 보장 기간, 배송 소요일)를 코드 수정 없이 바꾸는 JSON 설정 저장소. 대시보드 `/settings`가 읽고 쓰고, `product_pages/scripts/build_pages.py`가 `UNIFI_BRAND`/`GLINET_BRAND`/트러스트 템플릿 기본값 위에 덮어씌움. 이미 생성된 상세페이지엔 소급 적용 안 됨 |
...
- **`/settings`**: 브랜드 로고/문구, 구매·배송 안내 문구(초기불량 보장 기간/배송 소요일)를 코드 수정 없이 바꾸는 페이지 - `common_settings.py`/`common_settings.json`, `build_pages.py`가 병합해서 사용. 이미 만든 상세페이지엔 소급 적용 안 됨. (79)
...
- **상세페이지 제작 도구** (`/detail-page-builder`): 카피라이팅(태그라인/Why 3카드/Design/Tech Specs)만 폼에 채우면 HTML 조립·PNG export는 코드가 처리. 엔진은 `product_pages/scripts/build_detail_page.py`(브랜드 무관, "콘텐츠 브리프" dict 하나로 동작). 저장된 브리프는 `Product Pages_html/briefs/<slug>.json`에 남아 "저장된 브리프 불러오기"로 재사용 가능. NocoDB SKU 검색으로 브랜드/상품명/이미지폴더명 자동채움 - **검색 결과를 고르거나 저장된 브리프를 불러오면 이미지 갤러리도 자동으로 로드됨(2026-08-03)**, 예전엔 "이미지 불러오기" 버튼을 한 번 더 눌러야 했음. **페이지 섹션 순서(2026-08-03, 사용자 요청)**: "🤖 Claude로 브리프 초안 생성"이 "1) 기본 정보"보다 위에 옴(공홈 URL/참고자료로 초안부터 만든 뒤 기본 정보를 채우는 흐름이 자연스럽다는 판단) - `title_input` 등 "1) 기본 정보"의 위젯을 이 섹션의 버튼 핸들러가 참조하지만 파이썬 클로저가 실행 시점에 이름을 찾기 때문에 소스 순서를 바꿔도 문제 없음. "🤖 브리프 초안 생성" 버튼으로 Claude(`brief_generator.py`, CheapSub 중계 API 경유, **claude-opus-5** - 2026-08-03에 claude-sonnet-5에서 변경)가 공홈 URL만으로 초안을 만들어 폼을 자동으로 채워줌(생성 결과는 항상 초안 - 저장/업로드 전 사람이 검토·수정). GL.iNet 최초 상세페이지: Slate 7(`GLiNet Slate 7 (GL-BE3600).html`). **파일명 규칙(2026-08-04, 사용자 확정): `{SKU}.html`** - 예: `GLiNet Slate 7 (GL-BE3600).html`, `UniFi Pro Max 16.html`(기존 `{브랜드 supply_label} - {title}.dc.html` 형태에서 전면 변경, 확장자도 `.dc.html`→`.html`). NocoDB 검색으로 상품을 고르면 그 레코드의 SKU를 `state["sku"]`에 고정해두고, 파일명·에셋 slug(`assets/<slug>/`)·브리프 파일명(`briefs/<slug>.json`)·Product_Page 매칭(`_find_nocodb_record`) 전부 이 값을 쓴다 - `title_input.value`(사람이 마케팅 문구로 자유롭게 다듬는 값, 페이지 본문 표시에만 씀)를 식별자로 쓰면 타이틀을 다듬을 때마다 파일명이 바뀌고 매칭도 깨지는 문제가 실사용 중 발견됨(처음엔 Model Number를 식별자로 썼다가, 사용자가 최종적으로 SKU 기준 파일명을 요청해 다시 전환). `sku`가 없으면(NocoDB 검색 없이 자유 입력) title로 폴백. 매칭 레코드를 못 찾으면 로그에 경고만 남기고 생성 자체는 막지 않음. **기존 103개 파일도 전부 일괄 리네임 완료** - SKU 자동 매칭(브랜드 접두어 + 파일명의 " - " 뒷부분을 SKU와 정규화 비교) 87개 + 축약형 표기 차이(UCG→Cloud Gateway, USW 접두어 등)로 자동 매칭 안 된 15개 + GL.iNet 1개(파일 자체가 로컬/NAS 양쪽에서 사라져 있던 걸 발견 - 브리프 JSON과 이미지 에셋은 남아있어 `write_page()`로 재생성 후 마이그레이션)는 수동 확인해 103/103 전부 해결. 마이그레이션 스크립트는 각 파일의 "Compare 섹션 등에서 참조하는 다른 상품의 에셋 폴더"까지 다치지 않도록 전역 old-slug→new-slug 매핑을 만들어 전체 파일에 동시 적용(파일별로 자기 자신의 slug만 보고 치환하면 다른 파일이 참조하는 링크가 끊길 뻔했음). `smartstore.py`의 `_match_detail_page()`도 이제 SKU 정확일치로 단순화(예전엔 `Model_Number`라는 존재하지 않는 필드명으로 조회하고 있던 버그도 이 김에 발견/수정 - 실제 필드명은 공백 있는 `"Model Number"`). **PNG Export도 2026-08-03부터 my.tbd.kr에서 동작**(NAS Dockerfile에 `playwright` + `RUN playwright install --with-deps chromium` 추가, 아래 "Dockerfile의 pip install 목록" 항목 참고) - `export_sections.py`의 `chromium.launch()`에 `--no-sandbox`를 추가해야 했음(컨테이너가 root로 실행되는데 Chromium 자체 샌드박스가 root 권한 컨테이너에서 커널 네임스페이스 제약으로 기본 크래시하는 문제, 로컬에서도 문제 없이 동작하는 표준적 완화책이라 환경 분기 없이 항상 켜둠). 실제로 NAS 컨테이너 안에서 `export_sections.py`를 돌려 새 로고가 반영된 PNG까지 확인 완료. (68-69, 71)
