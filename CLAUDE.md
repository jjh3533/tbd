...
| `dashboard/` | NiceGUI 대시보드 (`theme.py`, `layout.py`, `components.py`(시맨틱 버튼/헤더/다이얼로그 헬퍼), `app.py`, `pages/{home,products,register,detail_page_builder,detail_page_editor,detail_page_common,inventory,needs_check,orders,sync,smartstore,settings,purchase_orders,shipping}.py`, `deploy/{Dockerfile,docker-compose.yml}`) |
...
| `brief_generator.py` | Claude(claude-opus-5, 2026-08-03에 claude-sonnet-5에서 변경)로 상세페이지 브리프(태그라인/Why 3카드/Design/Tech Specs) 초안을 생성. CheapSub 중계 API(`https://api.cheapsub.im`, Anthropic Messages API 호환) 경유, `CHEAPSUB_API_KEY` 필요(`.env`, 없으면 이 기능만 비활성화). 대시보드 `/detail-page-builder`의 "🤖 브리프 초안 생성" 버튼이 호출. **my.tbd.kr에도 배포됨** — NAS `Dockerfile`에 `anthropic` 패키지 추가(재빌드 필요) + NAS `.env`에 `CHEAPSUB_API_KEY` 설정 완료 |
...
| `common_settings.py`/`common_settings.json` | 브랜드 로고/문구를 코드 수정 없이 바꾸는 JSON 설정 저장소. 대시보드 `/settings`가 읽고 쓰고, `product_pages/scripts/build_pages.py`가 `UNIFI_BRAND`/`GLINET_BRAND` 기본값 위에 덮어씌움. 이미 생성된 상세페이지엔 소급 적용 안 됨. **구매·배송 안내 문구(초기불량 보장 기간 등)는 2026-08-04부로 이 파일이 다루지 않음** — `/detail-page-common`(공통영역 편집)에서 브랜드별 완성 HTML로 직접 편집하는 방식으로 대체됨(아래 "상세페이지 제작/편집/공통영역" 항목 참고) |
...
| `product_pages/scripts/` | 상세페이지 생성기 - 재사용 가능한 공용 도구: `build_pages.py`(신규 생성 패턴 - `HEAD`/`hero()`/`why_section()`이 `UNIFI_BRAND`/`GLINET_BRAND` 브랜드 딕셔너리를 파라미터로 받도록 확장됨, `trust_to_footer(brand, override_html=None)`은 `override_html`이 있으면 그대로 반환하고 없으면 기존 플레이스홀더 치환 템플릿으로 폴백), `build_detail_page.py`(브랜드 무관 "콘텐츠 브리프" dict → HTML 조립·저장·재읽기 + PNG export 엔진 + 공통영역 HTML 저장/불러오기(`read_common_footer`/`write_common_footer`), 대시보드 `/detail-page-builder`·`/detail-page-editor`·`/detail-page-common`이 전부 이 모듈을 직접 호출), `crop_hero.py`(히어로 이미지 반사 크롭 - Ubiquiti CDN 이미지 전용 로직이라 GL.iNet처럼 반사 없는 스튜디오 컷에 쓰면 오작동/과잉크롭함, 주의), `export_sections.py`(섹션별 PNG export, Playwright 필요). 카테고리별 1회성 배치 스크립트는 `archive/product_pages_scripts/`로 이동함 |
...
- **`/settings`**: 브랜드 로고/문구(헤더·히어로 로고 경로, eyebrow/store 표기 등)를 코드 수정 없이 바꾸는 페이지 - `common_settings.py`/`common_settings.json`, `build_pages.py`가 병합해서 사용. 이미 만든 상세페이지엔 소급 적용 안 됨. **구매·배송 안내 문구 섹션은 2026-08-04에 제거되고 `/detail-page-common`으로 이전됨**(아래 항목 참고). (79)
...
- **주문관리 발주/배송 (`/purchase`, `/shipping`)**: 새 NocoDB 테이블 `Order_Fulfillment`(`NOCODB_ORDER_TABLE_ID`)로 주문→발주→현지배송→배송대행지(에코트랜스 xlsx, `iecot_export.py`/`iecot_config.py`)→국제배송→네이버발송까지 추적. `naver_order_api.dispatch_product_order()`를 처음으로 실제 연결(미리보기 토글+확인 다이얼로그 필수). 실제 주문 1건으로 전 단계 검증 완료. **카카오 알림톡은 API 키/템플릿 승인 대기로 이번 범위에서 제외** - Phase D의 나머지 절반. **`/purchase`도 `/orders`와 동일하게 네이버 API를 직접 호출하지 않음(2026-08-04)** - 예전엔 페이지를 열 때마다 최근 7일을 네이버에서 직접 조회해서 느렸음(사용자 제보로 발견) - `Order_Fulfillment`만 읽도록 변경, 새 주문은 `/orders`의 동기화 버튼이 먼저 채워둬야 여기 보임. `/shipping`은 원래부터 페이지 로딩 시 네이버 호출이 없어서(발송 처리 버튼에서만 라이브 호출) 손댈 필요 없었음. (80)
...
- **대시보드 카운트**: `sync_engine.exclude_clone_rows()`로 Clone 로우를 제외한 실제 등록 상품(100개) 기준으로 집계(`home.py`/`products.py`/`needs_check.py`). `/inventory`처럼 색상별 추적이 목적인 곳만 원본 그대로 사용.
...
- **GL.iNet 브랜드**: GL.iNet 크롤링 시 SKU에 "GLiNet " 접두사 자동 추가(`register.py`의 `_BRAND_SKU_PREFIXES`). **GL.iNet 신규 상품 등록 시 NocoDB Category는 "WiFi"로 설정** — Brand 필드 기준 필터링(미설정 시 Category 폴백)이라 무관하게 정상 조회됨. **브랜드별 전용 라우트(`/brand/unifi`, `/brand/glinet`)는 2026-08-04에 제거되고 `/products`의 브랜드 필터로 흡수됨**(아래 "상품 리스트/Main Dashboard 분리" 항목 참고). (71)
...
- **상세페이지 제작/편집/공통영역 (2026-08-04 대개편)**: "➕ 신규등록" 하위메뉴 3개로 나뉨 — 브리프를 파일로 저장/불러오지 않고, "생성 → 코드 에디터로 마무리 → PNG export" 흐름으로 바뀜(브리프는 `state` dict 안에서 조립됐다가 버려짐, 예전 `save_brief`/`load_brief`/`Product Pages_html/briefs/` 전부 제거됨). 배경: 상세페이지 마지막 다듬기를 브리프 폼 재수정이 아니라 HTML 편집기로 직접 하고 싶다는 요청.
  - **① 상세페이지 제작** (`/detail-page-builder`): 카피라이팅(태그라인/Why 3카드/Design/Tech Specs) 폼 + NocoDB SKU 검색. 검색 결과를 고르면 브랜드/상품명(Model Number)/이미지폴더명 자동채움에 더해 **공홈 URL(Official_URL 필드)이 있으면 참고자료도 자동 크롤링**(2026-08-04, 예전엔 URL을 다시 붙여넣고 "가져오기"를 눌러야 했음). 이미지는 이 페이지에서 업로드하지 않고 "➕ 신규등록"(`/register`) 단계에서 미리 고른 걸 읽기만 한다(아래 항목 참고) - 파일명이 `-hero.`/`-design.`로 끝나면 자동 프리셋, `-store-N.`인 4장은 참고용 썸네일로만 표시, 접미사 없는 레거시 폴더는 예전처럼 사람이 버튼으로 직접 고름. "HTML 생성" 버튼을 누르면 파일이 만들어지고 완료 알림 + "✏️ HTML 편집" 버튼이 활성화되어(자동 이동은 안 하고 버튼으로 다음 페이지에 넘김) 계속 다른 상품을 이어 만들 수도 있음. **Product_Page 자동반영 로직은 이 단계에서 빠짐**(③ 참고).
  - **② 상세페이지 편집** (`/detail-page-editor`, 신규): NiceGUI 내장 `ui.codemirror`(HTML 하이라이팅, 추가 pip 설치 불필요)로 완성된 HTML을 그대로 불러와 그대로 저장 - Quill/TinyMCE 같은 WYSIWYG는 860px 고정폭+인라인 스타일 마크업을 재직렬화하며 깨뜨릴 위험이 있어 배제. "①"에서 방금 만든 파일은 `?file=` 쿼리파라미터로 자동 로드됨. "PNG로 Export" 버튼을 누르면 `export_sections.py`로 섹션별 PNG를 뽑고, **성공하면 그 시점에 처음으로 NocoDB `Product_Page`를 "Detail"로 반영**(파일명에서 확장자만 뗀 게 SKU라 `build_detail_page.find_nocodb_record()`로 매칭) - 예전엔 "HTML 생성" 시점에 반영했지만, 브리프만 만든 상태는 아직 "완성"이 아니고 편집+PNG export까지 끝나야 진짜 완성이라는 판단(사용자 확정). 미리보기 정적 서빙(`/detail-pages/...`)도 이 페이지가 등록.
  - **③ 공통영역 편집** (`/detail-page-common`, 신규): 상세페이지 하단 공통 섹션(TBD Seoul 신뢰뱃지~통관안내~배송/반품 안내~FAQ~Footer)을 브랜드별(UniFi/GL.iNet)로 **완성된 HTML 텍스트 그대로** `Product Pages_html/common/{unifi,glinet}-footer.html`에 저장 - 예전처럼 `build_pages.py`의 `__EYEBROW__`/`__STORE__` 플레이스홀더 치환 템플릿이 아니라, 저장된 파일을 그대로 이어붙이는 방식(`trust_to_footer(brand, override_html=...)`). 처음 열면 저장된 파일이 없어 지금 라이브에 나가는 폴백 템플릿으로 시드됨. 저장하면 **다음에 생성하는 상세페이지부터 반영**되고 이미 만든 페이지엔 소급 적용 안 됨(다른 공통 텍스트 변경과 동일한 관행 - 편집기로 직접 열어 고치거나 재생성 필요). 이 전환으로 `common_settings.py`의 `common_copy`(초기불량 보장 기간/배송 소요일)는 제거됨 - 이제 이 HTML 안에 직접 텍스트로 들어있음.
  - **이미지 역할 선택 다운로드** (`/register`): 공홈 크롤링 직후 이미지마다 히어로/디자인/스토어1~4 역할 버튼이 뜨고, "이미지 다운로드"를 누르면 역할별 파일명(`01-hero.ext`/`02-design.ext`/`03-store-1.ext`...)으로 저장됨 - 위 ①의 자동 프리셋이 이 파일명 패턴을 보고 동작. 역할 미지정 이미지는 다운로드 안 됨(히어로/디자인만 있어도 진행 가능).
  - **파일명 규칙(2026-08-04 이전 세션에 확정, 유지): `{SKU}.html`** - 예: `GLiNet Slate 7 (GL-BE3600).html`, `UniFi Pro Max 16.html`. NocoDB 검색으로 상품을 고르면 그 레코드의 SKU를 `state["sku"]`에 고정해 파일명·에셋 slug(`assets/<slug>/`)·Product_Page 매칭 전부 이 값을 쓴다(title은 사람이 자유롭게 다듬는 마케팅 문구라 식별자로 안 씀). `sku`가 없으면(자유 입력) title로 폴백.
  - **PNG Export는 my.tbd.kr(NAS)에서도 동작**(2026-08-03, NAS Dockerfile에 `playwright` + `RUN playwright install --with-deps chromium` 추가, 아래 "Dockerfile의 pip install 목록" 항목 참고) - `export_sections.py`의 `chromium.launch()`에 컨테이너 root 실행 대응 `--no-sandbox` 포함. (68-69, 71, 신규 세션 추가분)
- **Main Dashboard / 상품 리스트 분리(2026-08-04)**: `/`(home.py)에서 하단 "전체 상품" 표를 뺐다 - 이제 랜딩페이지(환율/Scrape.do/재고현황/페이지현황/스마트스토어현황 카드 + 카테고리 그리드)로만 씀. 신규 `/products`(`products.py`)가 그 표+필터를 흡수하고 `brand.py`의 검색/카테고리/상태/정렬 필터 로직에 브랜드 필터(전체/UniFi/GL.iNet)와 **상세페이지 유무 필터**(전체/Detail/Simple/없음, `Product_Page` 필드 기준)를 추가해 한 페이지에서 모든 조합 필터링이 되도록 함. 상단 4카드(상품 수/판매 가능/품절/확인 필요)는 필터와 무관하게 전체 기준. `category.py`(`/category/{slug}`)와 `brand.py`(`/brand/{slug}`)는 완전히 삭제되고 404 처리됨 - 카테고리 카드 클릭은 `/products?category={카테고리명}` 쿼리파라미터로 진입(`@ui.page`의 자동 쿼리 바인딩 이용, 슬러그 매핑 없이 카테고리명 그대로 URL-encode해서 넘김).
- **가격 업데이트 카드 개편(2026-08-04)**: 예전엔 카드 4장(상품 수/판매 가능/품절/확인 필요) 아래 별도 버튼 행("⚡ Sync All Retailers"/"🔍 Sync 확인 필요만"/리테일러별 버튼)이 있었는데, 이 버튼들을 전부 카드 안으로 흡수함. 1번째 줄: "상품 수" 카드에 전체 Sync 버튼, "확인 필요" 카드에 확인 필요만 Sync 버튼 내장(판매 가능/품절은 버튼 없음). 2번째 줄(신설): Adorama/Amazon/B&H 카드 각각 자체 Sync 버튼 + Scrape.do 크레딧 카드(가로형 도넛, 세로형과 별개 variant). `components.stat_card()`에 `button_label`/`on_click` 파라미터를 추가해 버튼을 카드 안에 내장하고(값을 반환해서 disable/enable 대상 버튼 목록에 호출부가 직접 모을 수 있게 함 - raw HTML 카드로는 파이썬 콜백을 못 묶어서 `ui.element`+`ui.button` 조합으로 바뀜), `components.scrapedo_donut_card()`에 `orientation="horizontal"`을 추가(도넛 왼쪽 + 숫자 오른쪽 배치, 세로형 Main Dashboard 호출부는 그대로 유지).
- **상세페이지 헤더/푸터 로고 교체(2026-08-03)**: Google Drive `TBD Seoul/Assets/`의 신규 로고(`unifi_tbd.svg`/`glinet_tbd.svg`/`tbd_logo_gray.svg`)를 `Product Pages_html/assets/common/`에 `common_logo-unifi-tbd.svg`/`common_logo-glinet-tbd.svg`/`common_logo-footer.svg`(덮어씀)로 반영. 헤더는 기존 "브랜드 로고 + 구분선 + 작은 TBD 로고" 2단 구성을 브랜드별 콜라보 단일 로고 1개로 교체(`build_pages.py`의 `UNIFI_BRAND`/`GLINET_BRAND`의 `header_logo_src` + `head()` 템플릿, `common_settings.py`의 `DEFAULTS`도 동일하게 동기화 - 안 그러면 `/settings`에 저장된 값이 없을 때 이 파일의 구버전 기본값이 다시 덮어씀). **공용 asset 파일(footer/hero처럼 상대경로로 참조되는 이미지)은 파일 내용만 바꾸면 이미 생성된 상세페이지에도 소급 적용됨**(텍스트 문구와 달리 이미지는 각 파일에 내용이 박히는 게 아니라 참조만 하기 때문) - 다만 헤더는 마크업 구조 자체(2단→1단)가 바뀌어서 기존 103개 파일을 별도 배치 스크립트로 일괄 치환함(신규 생성 페이지는 `build_pages.py` 수정만으로 자동 반영). (82)
...
- 네이버 스마트스토어 등록 상품: **100개**
  ...
  - Integrations: 6개
  - 기타: 1개
- 상세페이지(HTML) 보유: **103개** (Naver 등록 100개 + 미등록 3개: Display Cast Lite, Mobile Router Industrial, GL.iNet Slate 7)
...
- **사이드바 구조**: PRODUCTS(신규등록[하위: 상세페이지 제작/상세페이지 편집/공통영역 편집]/상품리스트/가격업데이트/품절변동/스마트스토어) → SALES(주문/발주/배송) → COMMON(공통 설정) → LINKS(TBD Seoul 스마트스토어/스마트스토어센터/커머스API센터). "➕ 신규등록"은 `/register`·`/detail-page-builder`·`/detail-page-editor`·`/detail-page-common` 중 하나가 active일 때만 하위 3개를 펼쳐 보여주는 부모 항목(2026-08-04, 예전엔 "상품 리스트"가 UniFi/GL.iNet 서브메뉴를 펼치는 패턴이었는데 그 자리를 이걸로 대체) - "📋 상품 리스트"는 `/products`로 연결(예전엔 `/`). LINKS는 외부 링크로 `target="_blank"`로 새 창 오픈, 활성 상태 없음. `dashboard/layout.py`의 `frame()` 함수에서 관리
